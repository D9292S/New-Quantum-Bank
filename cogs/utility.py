...
class Utility(commands.Cog):
    ...
    async def _show_general_help(self, ctx):
        ...
        for cog_name, commands in commands_by_cog.items():
            ...
            # Format command list
            command_list = ", ".join(f"`/{c.name}`" for c in commands)
            embed.add_field(
    ...
    async def _show_command_help(self, ctx, command_name: str):
        ...
        if command.options:
            ...
            for option in command.options:
                ...
                default = f" (Default: {option.default})" if hasattr(option, "default") and option.default is not None else ""
    ...
    async def info(self, ctx):
        ...
        embed.add_field(
            name="Credits",
            value="Created with ❤️ by TheCodingGuy",
            inline=False
        )
    ...
    async def invite(self, ctx):
        ...
        embed.add_field(
            ...
            value="[quantumbank.gg](https://quantumbank.gg)",
            inline=False
        )
    ...
    async def on_interaction(self, interaction):
        if interaction.type == discord.InteractionType.component:
            if interaction.data.get("custom_id") == "run_benchmark":
                ...
                results = await self._run_performance_benchmark()
    ...
    async def cog_command_error(self, ctx, error):
        ...
        if isinstance(error, discord.errors.ApplicationCommandInvokeError):
            await ctx.respond("There was an error processing your command. Please try again later.", ephemeral=True)
