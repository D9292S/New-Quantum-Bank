...
class AnimeView(discord.ui.View):
    def __init__(self, anime_data):
        ...
        self.add_item(discord.ui.Button(label="📺 View on MyAnimeList", url=f"https://myanimelist.net/anime/{anime_data.get('id', '')}"))
    ...
    async def select_callback(self, select, interaction):
        ...
        if select.values[0] == "status":
            ...
            await interaction.response.send_message(f"🎬 Episodes: `{y.get('num_episodes', 'Unknown')}`", ephemeral=True)
...
class Anime(commands.Cog):
    ...
    async def anime(self, ctx: discord.ApplicationContext, name: str):
        ...
        try:
            ...
            # Basic Information
            status_emoji = "🟢" if anime_data.get("status") == "finished_airing" else "🔴" if anime_data.get("status") == "currently_airing" else "⚪"
            basic_info = [
            ...
            embed.set_footer(text="Data from MyAnimeList", icon_url="https://cdn.myanimelist.net/img/sp/icon/apple-touch-icon-256.png")
