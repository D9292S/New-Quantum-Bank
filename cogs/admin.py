...
class Admin(commands.Cog):
    ...
    async def cog_load(self):
        ...
        self.logger.warning("Running in degraded mode - most admin features will return errors until database connection is restored")
    ...
    async def add_money(self, ctx, user_id: str, amount: float):
        ...
        if account.get('account_type') == 'savings':
            old_interest_rate = account.get('interest_rate', 2.5)
            new_interest_rate = self.db._calculate_interest_rate_by_balance(new_balance)
            interest_rate_info = f"Interest rate: {old_interest_rate}% → {new_interest_rate}%" if old_interest_rate != new_interest_rate else f"Interest rate: {old_interest_rate}%"
    ...
    async def set_interest_rate(self, ctx, user_id: str, interest_rate: float):
        ...
        if account.get('account_type') != 'savings':
            await ctx.respond(f"⚠️ This account is not a savings account. Converting to savings with {interest_rate}% interest rate.", ephemeral=True)
    ...
    async def calculate_all_interest(self, ctx):
        ...
        try:
            ...
            self.logger.error({
                'event': 'Interest calculation timeout',
                'level': 'error',
                'timeout': '60 seconds'
            })
