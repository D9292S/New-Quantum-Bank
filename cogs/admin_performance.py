...
class AdminPerformance(commands.Cog):
    def __init__(self, bot):
        ...
        self.periods = {"5m": 300, "15m": 900, "1h": 3600, "24h": 86400}
        ...
        if not hasattr(self.bot, "performance_monitor"):
            if mongo_cog and hasattr(mongo_cog, "performance_monitor"):
                ...
                try:
                    ...
                    self.bot.performance_monitor = PerformanceMonitor(self.bot)
                    self.logger.warning("Created fallback performance monitor")
                except ImportError:
                    self.logger.error("Could not create fallback performance monitor")
    ...
    async def _init_dependencies(self):
        ...
        while attempts < max_attempts:
            ...
            if attempts < max_attempts:
                self.logger.warning(
                    f"Performance monitor not found yet, retrying in {retry_delay}s ({attempts}/{max_attempts})"
                )
    ...
    async def resource_usage(self, ctx):
        ...
        try:
            ...
            embed.add_field(name="💾 Disk Usage", value="Error retrieving disk information", inline=False)
        ...
        try:
            if mode:
                ...
                self.bot.maintenance_message = (
                    message or "Bot is currently undergoing maintenance. Please try again later."
                )
                ...
                activity_status = (
                    getattr(self.bot.config, "ACTIVITY_STATUS", "Quantum Bank | /help")
                    if hasattr(self.bot, "config")
                    else "Quantum Bank | /help"
                )
    ...
    async def performance_metrics(
        self,
        ctx,
        period: discord.Option(str, "Time period", choices=["5m", "15m", "1h", "24h"]) = "15m",
    ):
    ...
    async def slow_operations(
        self,
        ctx,
        threshold: discord.Option(float, "Time threshold in ms", min_value=50.0, max_value=10000.0) = 500.0,
        count: discord.Option(int, "Number of operations to show", min_value=1, max_value=25) = 10,
    ):
        """Show the slowest operations recorded"""
        try:
            self.logger.info(
                f"Slow operations requested by {ctx.author.name} (threshold: {threshold}ms, count: {count})"
            )
    ...
    async def logs(
        ...
        category: discord.Option(
            str, "Log category", choices=["bot", "commands", "database", "performance", "errors"]
        ) = "bot",
    ...
    async def clear_logs(
        ...
        category: Option(
            str,
            "Log category",
            choices=["all", "bot", "commands", "database", "performance", "errors"],
        ) = "all",
    ):
