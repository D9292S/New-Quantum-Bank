...
class Cache:
    ...
    async def get(self, key: str) -> Any | None:
        ...
        try:
            ...
            self.logger.error({"event": "Cache get error", "error": str(e), "key": key, "level": "error"})
            return None
    ...
    async def set(self, key: str, value: Any) -> None:
        ...
        try:
            ...
            if cache_count >= self.max_size:
                ...
                await self.db.cache.delete_many({"expires_at": {"$lt": datetime.utcnow()}})
                # If still too many, remove oldest
                if await self.db.cache.count_documents({}) >= self.max_size:
                    await self.db.cache.delete_one({"created_at": {"$exists": True}}, sort=[("created_at", 1)])
            ...
            self.logger.error({"event": "Cache set error", "error": str(e), "key": key, "level": "error"})
    ...
    async def invalidate(self, key: str) -> None:
        ...
        try:
            ...
            self.logger.error({"event": "Cache invalidation error", "error": str(e), "key": key, "level": "error"})
    ...
    async def clear(self) -> None:
        ...
        try:
            ...
            self.logger.error({"event": "Cache clear error", "error": str(e), "level": "error"})
...
class Account(commands.Cog):
    ...
    async def cog_load(self):
        ...
        self.logger.error(
            "Failed to connect to database after maximum attempts, accounts cog will operate in limited mode"
        )
    ...
    async def _establish_direct_connection(self):
        ...
        self.mongo_client = AsyncIOMotorClient(
            mongo_uri, serverSelectionTimeoutMS=5000, connectTimeoutMS=5000, socketTimeoutMS=20000
        )
    ...
    async def register_command(self, ctx):
        ...
        try:
            ...
            if account:
                await ctx.respond(
                    f"You already have an account with balance of ${account['balance']}",
                    ephemeral=True,
                )
                return
    ...
    async def _calculate_interest_daily(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.error({"event": "Failed to get savings accounts", "error": str(e), "level": "error"})
    ...
    async def on_application_command_error(self, ctx, error):
        ...
        if isinstance(error, DatabaseError):
            embed = discord.Embed(title="Database Error", description=str(error), color=discord.Color.red())
            await ctx.respond(embed=embed, ephemeral=True)
            return

        if isinstance(error, ValidationError):
            embed = discord.Embed(title="Validation Error", description=str(error), color=discord.Color.red())
            await ctx.respond(embed=embed, ephemeral=True)
            return

        if isinstance(error, AccountError):
            embed = discord.Embed(title="Account Error", description=str(error), color=discord.Color.red())
            await ctx.respond(embed=embed, ephemeral=True)
            return

        if isinstance(error, AccountNotFoundError):
            embed = discord.Embed(title="Account Not Found", description=str(error), color=discord.Color.red())
            embed.add_field(
        ...
        if isinstance(error, AccountTypeError):
            embed = discord.Embed(title="Account Type Error", description=str(error), color=discord.Color.red())
            embed.add_field(
        ...
        if isinstance(error, AccountAlreadyExistsError):
            embed = discord.Embed(title="Account Already Exists", description=str(error), color=discord.Color.red())
            embed.add_field(
        ...
        if isinstance(error, TransactionError):
            embed = discord.Embed(title="Transaction Error", description=str(error), color=discord.Color.red())
            await ctx.respond(embed=embed, ephemeral=True)
            return

        if isinstance(error, InsufficientFundsError):
            embed = discord.Embed(title="Insufficient Funds", description=str(error), color=discord.Color.red())
            embed.add_field(
                name="Need a loan?",
        ...
        if isinstance(error, TransactionLimitError):
            embed = discord.Embed(title="Transaction Limit Reached", description=str(error), color=discord.Color.red())
            embed.add_field(
        ...
        if isinstance(error, InvalidTransactionError):
            embed = discord.Embed(title="Invalid Transaction", description=str(error), color=discord.Color.red())
            await ctx.respond(embed=embed, ephemeral=True)
            return
        ...
        if isinstance(error, PassbookError):
            embed = discord.Embed(title="Passbook Error", description=str(error), color=discord.Color.red())
            await ctx.respond(embed=embed, ephemeral=True)
            return

        if isinstance(error, KYCError):
            embed = discord.Embed(title="KYC Verification Error", description=str(error), color=discord.Color.red())
            embed.add_field(
                name="Need Help?",
        ...
        if isinstance(error, LoanError):
            embed = discord.Embed(title="Loan Error", description=str(error), color=discord.Color.red())
            await ctx.respond(embed=embed, ephemeral=True)
            return

        if isinstance(error, LoanLimitError):
            embed = discord.Embed(title="Loan Limit Error", description=str(error), color=discord.Color.red())
            embed.add_field(
                name="Credit Score",
        ...
        if isinstance(error, LoanRepaymentError):
            embed = discord.Embed(title="Loan Repayment Error", description=str(error), color=discord.Color.red())
            await ctx.respond(embed=embed, ephemeral=True)
            return

        if isinstance(error, LoanAlreadyExistsError):
            embed = discord.Embed(title="Loan Already Exists", description=str(error), color=discord.Color.red())
            embed.add_field(
        ...
        if isinstance(error, CreditScoreError):
            embed = discord.Embed(title="Credit Score Error", description=str(error), color=discord.Color.red())
            await ctx.respond(embed=embed, ephemeral=True)
            return

        if isinstance(error, InsufficientCreditScoreError):
            embed = discord.Embed(title="Low Credit Score", description=str(error), color=discord.Color.red())
            embed.add_field(
    ...
    async def _check_daily_transaction_limit(self, user_id: str, amount: float) -> bool:
        ...
        try:
            ...
            daily_total = sum(
                txn["amount"] for txn in transactions if txn["timestamp"] >= today and txn["type"] == "send_upi_payment"
            )
    ...
    async def view_transactions(self, ctx):
        ...
        transaction_list = "\n".join(
            [f"{txn['type'].capitalize()} 💵: ${txn['amount']} on {txn['timestamp']}" for txn in transactions]
        )
    ...
    async def apply_loan(
        self,
        ctx,
        amount: discord.Option(float, "Amount to borrow", min_value=100),
        ...
        if account_age < 7:
            raise LoanError(
                f"Your account must be at least 7 days old to apply for a loan. Your account is {account_age} days old."
            )
        ...
        embed.add_field(name="Monthly Payment", value=f"${monthly_payment:,.2f}", inline=True)
        ...
        confirm_button = discord.ui.Button(
            style=discord.ButtonStyle.green, label="Confirm Loan", custom_id="confirm_loan"
        )
        ...
        try:
            interaction = await self.bot.wait_for(
                ...
                and i.data.get("custom_id") in ["confirm_loan", "cancel_loan"],
                timeout=120.0,
            )
    ...
    async def repay_loan(
        self,
        ctx,
        amount: discord.Option(float, "Amount to repay (defaults to monthly payment)", required=False) = None,
    ):
        ...
        try:
            ...
            if payment_result["fully_paid"]:
                ...
                embed.add_field(name="Amount Paid", value=f"${payment_result['amount_paid']:,.2f}", inline=True)

                embed.add_field(name="Remaining Balance", value="$0.00", inline=True)

                embed.add_field(name="Loan Status", value="Paid ✅", inline=True)
                ...
                embed.add_field(name="Loan Status", value="Active", inline=True)
    ...
    async def loan_status(self, ctx):
        ...
        if loan_status["status"] == "paid":
            ...
            embed.add_field(name="Original Amount", value=f"${loan_status['amount']:,.2f}", inline=True)

            embed.add_field(name="Interest Rate", value=f"{loan_status['interest_rate']}%", inline=True)

            embed.add_field(name="Term", value=f"{loan_status['term_months']} months", inline=True)

            embed.add_field(name="Paid On", value=f"{loan_status['end_date'].strftime('%Y-%m-%d')}", inline=True)
            ...
            embed.add_field(name="Loan Amount", value=f"${loan_status['amount']:,.2f}", inline=True)

            embed.add_field(name="Interest Rate", value=f"{loan_status['interest_rate']}%", inline=True)

            embed.add_field(name="Term", value=f"{loan_status['term_months']} months", inline=True)

            embed.add_field(name="Monthly Payment", value=f"${loan_status['monthly_payment']:,.2f}", inline=True)
            ...
            embed.add_field(name="Progress", value=f"{loan_status['progress_percent']:.1f}%", inline=True)
    ...
    async def loan_calculator(
        self,
        ctx,
        amount: discord.Option(float, "Loan amount", min_value=100),
        ...
        if comparisons:
            embed.add_field(name="Term Comparisons", value="\n".join(comparisons), inline=False)
    ...
    async def credit_score(self, ctx):
        ...
        embed.add_field(name="Score Range", value=score_meter, inline=False)
    ...
    async def credit_report(self, ctx):
        ...
        try:
            ...
            if recent_history:
                ...
                embed.add_field(name="Recent Credit History", value=history_text, inline=False)
    ...
    async def balance_command(self, ctx):
        ...
        try:
            ...
            if not account:
                await ctx.respond(
                    "You don't have a bank account yet! Use `/register` to create one.",
                    ephemeral=True,
                )
                return
            ...
            embed.add_field(
                name="Account Age",
                value=self._get_account_age(account.get("created_at")),
                inline=True,
            )
