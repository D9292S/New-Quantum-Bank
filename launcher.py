...
{COLORS["BOLD"]}{COLORS["GREEN"]}               ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝  ╚═╝{COLORS["RESET"]}
    ...
    print(banner)
    print(
        f"{COLORS['BOLD']}⚡ {COLORS['BLUE']}Quantum Bank Bot{COLORS['RESET']} {COLORS['GREEN']}v{__version__}{COLORS['RESET']}"
    )
    print(
        f"{COLORS['GRAY']}Running on {COLORS['BLUE']}Python {platform.python_version()}{COLORS['RESET']} | {COLORS['YELLOW']}{platform.system()} {platform.release()}{COLORS['RESET']}"
    )
...
def run_bot():
    ...
    if performance_mode == "high":
        ...
        try:
            ...
            if not sys.platform.startswith("win"):
                ...
                print(
                    f"{ColoredFormatter.COLORS['GREEN']}✓ Using uvloop for improved event loop performance{ColoredFormatter.COLORS['RESET']}"
                )
                uvloop.install()
        except ImportError:
            print(
                f"{ColoredFormatter.COLORS['YELLOW']}✗ uvloop not available - using standard asyncio event loop{ColoredFormatter.COLORS['RESET']}"
            )
    ...
    try:
        ...
        print(
            f"{ColoredFormatter.COLORS['GRAY']}─────────────────────────────────────────────────────────────{ColoredFormatter.COLORS['RESET']}"
        )
        bot_instance.run()
