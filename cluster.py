...
class BotCluster:
    ...
    def __init__(self, cluster_count: int, shard_count: int, launcher_path: str, 
                 restart_delay: int = 5):
    ...
    def start_cluster(self, cluster_id: int):
        ...
        cmd = [
            ...
            "--clusters", str(self.cluster_count),
            "--shards", str(self.shard_count),
            "--shardids", shard_ids_str
        ]
    ...
    def show_status(self):
        ...
        print(f"System Memory: {system['memory_percent']}% used ({system['memory_available_mb']} MB free of {system['memory_total_mb']} MB)")
        
        # Print cluster stats
        print("\nClusters:")
        print("-" * 80)
        print(f"{'ID':^4} | {'Status':^10} | {'PID':^8} | {'Uptime':^15} | {'Memory (MB)':^12} | {'CPU %':^7} | {'Shards':^20}")
        print("-" * 80)
        ...
        for cluster_id, cluster in sorted(status['clusters'].items()):
            ...
            print(f"{cluster_id:^4} | {status_str:^10} | {pid:^8} | {uptime:^15} | {memory:^12} | {cpu:^7} | {shards:^20}")
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=int, 
        default=1, 
        help="Total number of shards across all clusters"
    )
    parser.add_argument(
        "--launcher", "-l", 
        type=str, 
    ...
    parser.add_argument(
        "--status", 
        action="store_true",
        help="Show status of running clusters and exit"
    )
    parser.add_argument(
        "--status-interval",
        type=int,
        default=0,
        help="Show status every N seconds (0 = disabled)"
    )
    parser.add_argument(
        "--run-cluster",
        type=int,
