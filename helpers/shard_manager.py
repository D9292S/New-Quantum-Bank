...
class ShardManager:
    ...
    async def get_all_shard_statuses(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            cursor = self.mongodb.shard_status.find({
                "last_updated": {"$gt": cutoff_time}
            })
    ...
    async def send_cross_shard_event(self, event_type: str, data: Dict[str, Any], 
        ...
        try:
            ...
            for event in events:
                ...
                await self.mongodb.shard_events.update_one(
                    {"_id": event["_id"]},
                    {"$addToSet": {"processed_by": self._cluster_id}}
                )
            ...
            await self.mongodb.shard_events.delete_many({
