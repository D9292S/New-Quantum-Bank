...
class CacheManager:
    ...
    async def cleanup(self):
        ...
        async with self._lock:
            ...
            for namespace in list(self._memory_cache.keys()):
                ...
                # Find expired keys
                expired_keys = [key for key, item in data.items() if item.get("expires_at", 0) < now]
        ...
        if self._mongodb is not None:
            try:
                ...
                cache_data = await self._mongodb.cache.find_one(
                    {"namespace": namespace, "key": key, "expires_at": {"$gt": now}}
                )
    ...
    async def delete(self, key: str, namespace: str = "default") -> None:
        ...
        if self._mongodb is not None:
            try:
                await self._mongodb.cache.delete_one({"namespace": namespace, "key": key})
            except Exception as e:
    ...
    async def invalidate_namespace(self, namespace: str) -> None:
        ...
        if self._mongodb is not None:
            try:
                await self._mongodb.cache.delete_many({"namespace": namespace})
            except Exception as e:
